...
@section('content')
    ...
    <form class="" method="POST" id="job-project-form" action="{{ route('job-services.update', $service->id) }}">
        @csrf
        @method('PUT')
        <div class="row">
                ...
                <div class="card">
                    ...
                    <div class="card-body border-bottom-dashed">
                        ...
                        <div class="row gy-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('service_type') is-invalid @enderror"
                                        ...
                                        @foreach ($service_categories as $service_category)
                                            <option
                                                {{ $service_category->id == $service->service_category_id ? 'selected' : null }}
                                                value="{{ $service_category->id }}">{{ $service_category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label for="service-type-field" class="form-label">Service Type</label>
                                    <input type="text" class="form-control @error('service_type') is-invalid @enderror"
                                        name="service_type" id="service-type-field" value="{{ $service->service_type }}">
                                    @error('service_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="service_description" id="description-field" cols="30" rows="5"
                                        class="form-control @error('service_type') is-invalid @enderror">{{ $service->service_description }}</textarea>
                                ...
                                <div class="form-group mb-3">
                                    <div class="form-check form-switch mb-2">
                                        <input class="form-check-input" name="status" type="checkbox" role="switch"
                                            id="status-field" {{ $service->status == 'active' ? 'checked' : null }}>
